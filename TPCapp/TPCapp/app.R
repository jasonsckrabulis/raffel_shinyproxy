...
ui <- fluidPage(
    ...
    h3("This Shiny-R app was developed to enable users to fit metabolic theory of ecology (MTE) based thermal performance curves for any normally or lognormally distributed response variable. Not optimized for mobile devices."),
    hr(),
    ...
    tabsetPanel(id="tabset",
        ...
        # Analysis
        tabPanel("Analysis",
            h4("Upload a .csv file and receive an MTE-based thermal performance curve output by following the directions in the sidebar below. The analysis defaults to an Arrhenius model with log-normal error distribution no mass-scaling."),
            h4("Arrhenius Model:     Response = Ae^{Ea/(k*T)}"),
            h4("Log-transformed Arrhenius Model:     Ln(Response) = Ln(A) - Ea*(1/(k*T))"),
            hr(),
            ...
            sidebarLayout(
                ...
                sidebarPanel(
                    ...
                    helpText("Your .csv needs to be formatted with the following column headers as written: 'temperature', 'mass', 'response' (all lowercase and no single quotations). Temperatures should be in Celsius. If your data does not contain organism mass values (i.e., chemical reaction rate data), please insert mass=1 for all datapoints (the mass-correction option will not change your model output if this is the case)."),
        ...
        tabPanel("Notes and Help",
            ...
            mainPanel(
                ...
                p("Boltzmann's constant in this formulation is converts temperature into energy as 8.62*10^-5 eV/K."),
        ...
        tabPanel("Development",
            ...
            mainPanel(
                ...
                h4("Plotting 95% confidence bands"),
                ...
                h4("Responsive design for mobile"),
                hr(),
...
server <- function(input, output) {
        ...
        if(input$normalCheck==TRUE){
            ...
            if(input$massCheck==TRUE){
                ...
                arrPlotInv<-ggplot(data=dat,aes(x=1/(k*(temperature+K)),y=lnmassresponse))+
                    ...
                    labs(x=expression(paste("Inverse temperature ",bgroup("(",over(1,'kT'),")"))),y="Ln Mass-corrected Response")+
                    draw_label(label=paste0("Ln Mass-corrected Response = Ln(",round(summary(arr)$coef[1,1],digits=3),") - ",round(summary(arr)$coef[2,1],digits=3)," * {1/(kT)}"),x=min(1/(k*(dat$temperature+K))),y=min(dat$lnmassresponse),hjust=0,vjust=0)+
                    theme_cowplot(12)
                
                # Parameter estimates
                arrTable<-data.frame("Parameter"=c("A","Ea","AICc"),"Estimate"=c(round(summary(arr)$coef[1,1],digits=3),paste0(round(summary(arr)$coef[2,1],digits=3)," eV"),round(arrAICc,digits=3)),"Standard Error"=c(round(summary(arr)$coef[1,2],digits=3),round(summary(arr)$coef[2,2],digits=3),"--"))
                ...
                arrPlotInv<-ggplot(data=dat,aes(x=1/(k*(temperature+K)),y=lnresponse))+
                    geom_point(size=2)+
                    stat_function(fun=function(x)log(coef(arr)[1]*exp(-coef(arr)[2]*x)),geom="line",color="black",size=1,linetype="solid")+
                    labs(x=expression(paste("Inverse temperature ",bgroup("(",over(1,'kT'),")"))),y="Ln Response")+
                    draw_label(label=paste0("Ln Response = Ln(",round(summary(arr)$coef[1,1],digits=3),") - ",round(summary(arr)$coef[2,1],digits=3)," * {1/(kT)}"),x=min(1/(k*(dat$temperature+K))),y=min(dat$lnresponse),hjust=0,vjust=0)+
                    theme_cowplot(12)
    ...
    output$tinK<-renderText({
        if(v$doPlot==FALSE){
            return()
        } else{
            "*In the above figure, temperature T is modeled in Kelvin"
        }
    })
